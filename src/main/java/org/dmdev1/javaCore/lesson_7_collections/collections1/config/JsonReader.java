...
import org.dmdev1.javaCore.lesson_7_collections.collections2.Movie;
...
public class JsonReader {
    ...
    public static List<Movie> readMoviesFromJson(String fileName) {
        ObjectMapper objectMapper = new ObjectMapper();
        try (InputStream inputStream = JsonReader.class.getClassLoader().getResourceAsStream(fileName)) {
            if (inputStream == null) {
                throw new RuntimeException("File not found: " + fileName);
            }
            return objectMapper.readValue(inputStream, new TypeReference<List<Movie>>() {});
        } catch (Exception e) {
            throw new RuntimeException("Error reading JSON file", e);
        }
    }
